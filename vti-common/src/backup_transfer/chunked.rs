...
//!
//! The `stream` algorithm moves a bundle over the node's HTTPS blob endpoint,
//! which a node reachable only over DIDComm or TSP does not have. This one moves
//! it as a sequence of Trust Tasks over whatever transport carries the control
//! plane: `get-chunk` pulls an export chunk by index, and `put-chunk` writes an
//! import chunk checked against a manifest the operator committed before any
//! chunk moved. The normative account is `backup/initiate-export/0.1` § Chunked
//! transfer (trustoverip/dtgwg-trust-tasks-tf#633), generalised from the
//! agent's `vta/backup/initiate-export/1.1` (#474).
//!
//! Node-neutral: it stages and moves bytes a node has already encrypted. Making
//! those bytes — `initiate-export`'s serialization of the node's state — is the
//! node's own, and calls [`stage_export`] with the result.
//!
...
use super::bundle_store::{
    self as backup_bundle_store, BundleKind, BundleRecord, BundleState, mint_token,
};
use super::{
    MAX_BUNDLE_TTL_SECS, bundle_ttl, enforce_kind, enforce_open_bundle_cap, parse_bundle_id,
    require_owned, sha256_hex,
};
...
/// Stage already-encrypted export bytes as a chunked bundle.
///
/// The node's `initiate-export` serializes and encrypts its state, checks the
/// caller's authority and the open-bundle cap (see [`check_initiate`]), then
/// hands the bytes here. Needs no public URL: the chunks travel over the
...
pub async fn stage_export(
    ...
    tokio::fs::write(&blob_path, bytes)
        ...
        .map_err(AppError::Io)?;
    #[cfg(unix)]
    super::set_file_mode_600(&blob_path).await?;
...
/// The preconditions of any `initiate-*`: an unrestricted administrator, under
/// the open-bundle cap. Checked before the node serializes anything, so a
/// refused request costs no encryption work.
pub async fn check_initiate(ks: &KeyspaceHandle, auth: &AuthClaims) -> Result<(), ChunkedError> {
    auth.require_super_admin()?;
    enforce_open_bundle_cap(ks, &auth.did).await?;
    Ok(())
}

/// `complete-export` for a chunked bundle: release it and delete its staged
/// bytes now rather than at expiry. Returns `downloaded` — whether every index
/// was served at least once.
///
/// A chunked bundle is never one-shot: it stays `ExportReady` while its chunks
/// are served, and this is what ends it.
pub async fn complete_export(
    ks: &KeyspaceHandle,
    auth: &AuthClaims,
    bundle_id: &str,
) -> Result<bool, ChunkedError> {
    auth.require_super_admin()?;
    let id = parse_bundle_id(bundle_id)?;
    let _guard = chunk_lock().lock().await;
    let (mut record, plan) = load_chunked(ks, &id, &auth.did, BundleKind::Export).await?;
    if record.state != BundleState::ExportReady {
        return Err(ChunkedError::TerminalState("expired".into()));
    }
    let downloaded = plan.remaining() == 0;
        ...
        && e.kind() != std::io::ErrorKind::NotFound
    {
        tracing::warn!(
            ...
            path = %path.display(),
            error = %e,
            "complete-export (chunked): failed to delete staged bytes; sweeper will retry"
        );
        record.blob_path = Some(path);
    }
    record.state = BundleState::ExportAcked;
    backup_bundle_store::store_bundle(ks, &record).await?;
    delete_plan(ks, &id).await?;
    info!(bundle_id = %id, downloaded, "complete-export (chunked): released");
    Ok(downloaded)
}
...
async fn prepare_blob_path(blob_dir: &Path, id: &Uuid) -> Result<PathBuf, AppError> {
    tokio::fs::create_dir_all(blob_dir)
        ...
        .map_err(AppError::Io)?;
    #[cfg(unix)]
    super::set_dir_mode_700(blob_dir).await?;
...
async fn write_range(
    ...
    if !existed {
        #[cfg(unix)]
        super::set_file_mode_600(path).await?;

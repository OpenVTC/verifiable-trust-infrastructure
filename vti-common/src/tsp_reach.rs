//! Learn-from-inbound TSP reachability.
//!
//! A `did:key` peer — a device, a wallet — can't advertise a `#tsp` service in
//! a document, and it picks its inbox transport at runtime (the mobile app's
//! DIDComm/TSP toggle), so a node can't discover from the document whether the
//! peer is *currently* reachable over TSP. Instead it **learns from inbound**:
//! TSP `unpack_bytes` yields a cryptographically-proven `sender_vid`, so any
//! TSP frame a peer sends is proof that DID is, right now, listening on TSP.
//! A node's inbound dispatcher records that here; its push paths
//! (`crate::trust_task_push`, and the VTA's device push) read it to prefer
//! TSP over DIDComm for a recently-seen DID. Shared by the VTA and the VTC.
//!

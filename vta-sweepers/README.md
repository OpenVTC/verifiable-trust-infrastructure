...
`vta-service` so the periodic-maintenance logic lives below the service rather
than inside it.
...
storage-thread sweep loop in `server.rs` and the other call sites are unchanged.

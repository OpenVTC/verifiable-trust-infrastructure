...
| **An agent reading history through its VTA** | **Not yet.** `rooms/keys/open` resolves from the chain the member's VTA has accrued by applying commits, so a member who was present throughout is fine. A member who *joined* has the rungs in their client and no specified way to put them into their VTA — that needs another spec round (design note §12.2) |

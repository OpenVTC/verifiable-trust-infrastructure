//! Census: no two `ToSchema` types that can reach the VTA's OpenAPI document
//! share a schema name.
//!
...
//! `extend`, so two types with one name do not fail anything: the last one
//! registered wins, and every `$ref` to the name describes that one. The
//! rationale, the scanning technique and the incident that prompted it (#1697)
//! are written up on the twin of this test,
//! `vtc-service/tests/openapi_schema_names.rs`, which holds the same rule for
//! the VTC's document. The two are kept separate because each document has its
//! own inputs: this crate plus every workspace crate it depends on by path.
//!
//! When this was written the VTA's document had two such pairs, both a route
//! module re-declaring a vta-sdk wire type field for field
//! (`RollbackResponse`, `EnrollPasskeyChallengeBody`). The shapes matched, so
//! the document happened to be right; the copies were deleted in favour of the
//! vta-sdk types rather than left to drift apart under one name.
...
fn every_schema_name_in_the_document_inputs_is_defined_once() {
    ...
    assert!(
        ...
        "{} OpenAPI schema name(s) are defined by more than one type:\n\n{}\n\n\
         ...
         registered last — and that is what the served `/openapi.json` publishes. \

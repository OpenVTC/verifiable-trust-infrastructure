...
//! speaks — TSP, then DIDComm, then REST.
//!
//! # Why
//!
//! Every push this service originates — a removal notice, a consent request, a
//! granted notice — used to go out over DIDComm, whatever the recipient
//! advertised. The workspace rule is the reverse: the DID document is
//! authoritative for which protocols a party speaks, and the protocol used is
//! the highest-preference one present in both parties' documents, TSP over
//! DIDComm over REST (`docs/05-design-notes/tsp-enablement.md`). This module is
//! that rule for pushes.
//!
//! # How
//!
...
//!    (a `did:key` wallet) is reached over DIDComm through the shared mediator,
//!    as every member push was before. One that advertises only transports this
//!    service does not speak is refused, never silently downgraded.
//! 2. **Records** the push in the `member_pushes` keyspace — encrypted at rest
//!    under the VTC's storage key, like the install and passkey stores — with
//!    the document, the plan, and the deadline.
...
//!   (class 3, both DIDComm and TSP when the recipient is on this service's
...
pub(crate) const TSP_TRANSPORT_ID: &str = "member-push-tsp";
pub(crate) const REST_TRANSPORT_ID: &str = "member-push-rest";
...
/// The `TrustTaskHTTPS` service endpoint, if advertised. REST reaches a member
...
/// What this service can send over right now.
struct Ours {
    tsp: bool,
    didcomm: bool,
    rest: bool,
...
fn ours(state: &AppState) -> Ours {
    let messaging_up = state.didcomm.get().is_some();
    Ours {
        tsp: messaging_up && cfg!(feature = "tsp"),
...
/// reached over DIDComm through the shared mediator, which is how every member
/// push reached it before this module. One that advertises transports this
/// service cannot use is refused: never a silent downgrade past what the peer
/// said it speaks.
async fn plan(state: &AppState, recipient: &str) -> Result<(Vec<Protocol>, Reach), AppError> {
    let reach = resolve_reach(state, recipient).await;
    let ours = ours(state);
    ...
    if plan.is_empty() {
        return Err(AppError::Validation(format!(
            "no matching protocol for {recipient}: it advertises {} and this community can \
             send over {}",
...
async fn resolve_reach(state: &AppState, recipient: &str) -> Reach {
...
pub async fn push_trust_task(
    ...
    let (plan, reach) = plan(state, recipient).await?;
    ...
    loop {
        match queue_attempt(state, &mut record).await {
            Ok(()) => break,
            Err(e) if !record.remaining.is_empty() => {
                warn!(
                    ...
                    via = %record.current,
                    error = %e,
                    "could not queue a member push on its preferred transport; trying the next"
                );
    ...
    info!(recipient, via = %record.current, push = %id, "member push queued");
    Ok(id)
}
...
pub async fn outcome(
    ...
    Ok(load_record(&state.member_pushes_ks, id)
        .await?
...
async fn queue_attempt(state: &AppState, record: &mut PushRecord) -> Result<(), AppError> {
    let messaging = state.didcomm.get();
    let now = now_ms();
    ...
    let messaging = messaging
        .ok_or_else(|| AppError::Internal("VTC messaging not running — cannot push".into()))?;
...
async fn store_record(state: &AppState, record: &PushRecord) -> Result<(), AppError> {
    state
        .member_pushes_ks
        .insert(record_key(&record.id), record)
        .await
}
...
pub async fn sweep(state: &AppState) -> Result<(), AppError> {
    ...
    let Some(messaging) = state.didcomm.get() else {
        return Ok(());
    };
    let outbox = vti_common::outbox_store::VtiOutboxStore::new(state.outbox_ks.clone());
    let now = now_ms();
...
async fn escalate(state: &AppState, record: &mut PushRecord) -> Result<(), AppError> {
    if now_ms() >= record.deadline_ms {
        return finish(state, record, false, "none").await;
    }
    // A recipient that now offers nothing we speak leaves nothing to escalate to.
    let (fresh, reach) = plan(state, &record.recipient).await.unwrap_or_default();
    ...
    warn!(
        ...
        from = %tried,
        to = %next[0],
        "member push produced no delivery evidence in its window; escalating"
    );
    record.remaining = next;
    record.current = record.remaining.remove(0);
    record.attempt += 1;
    if let Err(e) = queue_attempt(state, record).await {
...
async fn finish(
    ...
    if delivered {
        ...
        warn!(
            ...
            "member push could not be confirmed on any transport the recipient offers"
        );
    }
    store_record(state, record).await
}
...
/// Sends a member push over TSP, sealed at send time from the encrypted push
/// record, and reports the collection-evidence id the mediator will list.
///
/// A recipient on this service's mediator is sealed and routed through it, and
/// the returned `hop_id` is the id the mediator lists the message under in
/// this service's outbox — `sha256` of the stored (base64url) form of the
...
#[cfg(feature = "tsp")]
pub(crate) struct TspPushTransport {
    pub(crate) atm: Arc<affinidi_tdk::messaging::ATM>,
    pub(crate) profile: Arc<affinidi_tdk::messaging::profiles::ATMProfile>,
    pub(crate) mediator_did: String,
    pub(crate) pushes: KeyspaceHandle,
...
/// Sends a member push over the HTTPS binding: `POST {base}/trust-tasks` with
/// the signed document as the body (HTTPS binding 0.2 §2, §6.1). No bearer
/// token — this service holds none for the recipient's server — so the
/// document's own proof is what authenticates it (§2 item 2).
pub(crate) struct RestPushTransport {
    http: reqwest::Client,
...
impl RestPushTransport {
    pub(crate) fn new(pushes: KeyspaceHandle) -> Self {
        let (tx, rx) = watch::channel(ConnState::Connected);
        Self {
            http: crate::recognition::verify::foreign_fetch_client(),
            pushes,
            _conn_tx: tx,
            conn: rx,
        }
